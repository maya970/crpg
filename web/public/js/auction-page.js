...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
      ...
      if (!s.logged_in) {
        g.toast('Connect your wallet from the top bar first.');
        return;
      }
      if (!s.has_hero) {
        g.toast('Create your character from the top bar, then refresh this page.');
        return;
      }
    } catch (e) {
      g.toast(String((e && e.message) || e || 'Could not reach the game'));
      return;
    }
    try {
